...
class DatabaseHandler(context: Context) : SQLiteOpenHelper(context, DatabaseHandler.DB_NAME, null, DatabaseHandler.DB_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        val CREATE_TABLE = "CREATE TABLE $TABLE_NAME ($ID INTEGER PRIMARY KEY, $NAME TEXT,$DESC TEXT,$COMPLETED TEXT,$SN TEXT,$SEMESTRE TEXT,$FECHA TEXT);"
    ...
    fun addTask(tasks: Tasks): Boolean {
        ...
        values.put(SN, tasks.sn)
        values.put(SEMESTRE, tasks.semestre)
        values.put(FECHA, tasks.fecha)
    ...
    fun getTask(_id: Int): Tasks {
        ...
        tasks.sn = cursor.getString(cursor.getColumnIndex(SN))
        tasks.semestre = cursor.getString(cursor.getColumnIndex(SEMESTRE))
        tasks.fecha = cursor.getString(cursor.getColumnIndex(FECHA))
        cursor.close()
        return tasks
    }
